...
public interface MessageRepository extends JpaRepository<Message, String> {
    ...
    @Query(
            value =
                    "SELECT * FROM messages WHERE room_id = :roomId ORDER BY timestamp DESC LIMIT :limit OFFSET :offset",
            nativeQuery = true)
    List<Message> findByRoomIdWithPagination(
            @Param("roomId") String roomId, @Param("offset") int offset, @Param("limit") int limit);
    ...
    @Query(
            "SELECT COUNT(m) FROM Message m WHERE m.room.id = :roomId AND m.receiverId = :receiverId AND m.read = false")
    ...
    @Modifying
    @Query(
            "UPDATE Message m SET m.read = true WHERE m.room.id = :roomId "
