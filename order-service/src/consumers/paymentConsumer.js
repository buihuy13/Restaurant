...
const startPaymentConsumer = async () => {
    try {
        ...
        await rabbitmqConnection.consumeMessage(rabbitmqConnection.queues.PAYMENT_COMPLETED, async (message) => {
        ...
        await rabbitmqConnection.consumeMessage(rabbitmqConnection.queues.PAYMENT_FAILED, async (message) => {
