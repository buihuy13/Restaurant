...
public class resService {
    ...
    public Mono<List<resResponseWithProduct>> getAllRestaurants(
            Coordinates location, String search, Integer nearby, String rating, String category) {
                            ...
                            .filter(
                                    r ->
                                            r.getCategories().stream()
                                                    .map(c -> c.getCateName().toLowerCase())
                                                    .toList()
        ...
        if (rating != null && !rating.isEmpty()) {
            if (rating.equals("asc")) {
                ...
                res =
                        res.stream()
                                .sorted(Comparator.comparing(restaurants::getRating).reversed())
                                .toList();
            }
        }
            ...
            return distanceService
                    ...
                    .map(
                            response -> {
                                ...
                                return IntStream.range(0, filteredRes.size())
                                        .mapToObj(
                                                i -> {
                                                    restaurants resIndex = filteredRes.get(i);

                                                    resResponseWithProduct resResponseIndex =
                                                            resUtil.mapResToResResponseWithProduct(
                                                                    resIndex);
    ...
    public Mono<resResponseWithProduct> getRestaurantById(String id, Coordinates location) {
        ...
        if (location != null) {
            ...
            return distanceService
                    .getDistanceAndDuration(start, end)
                    .map(
                            response -> {
                                if (response == null || response.getFeatures().isEmpty()) {
                                    throw new DistanceDurationException(
                                            "Error while calculating distance and duration");
                                }
                                Summary summary =
